...
// 验证ip
const ip = (rule, value, callback) => validator(rule, value, callback, async (value) => {
  let msg = !Tool.isIp(value) ? 'IP格式有误' : '';
  rule.isUnique && (await Api.get('test')).data && (msg = 'IP已存在');
  return msg;
});

// 验证域名
const domain = (rule, value, callback) => validator(rule, value, callback, async (value) => {
  let msg = !Tool.isDomain(value) ? '域名格式有误' : '';
  rule.isUnique && (await Api.get('test')).data && (msg = '域名已存在');
  return msg;
});

// 验证HASH
const hash = (rule, value, callback) => validator(rule, value, callback, async (value) => {
  let msg = !Tool.isHash(value) ? 'HASH格式有误' : '';
  rule.isUnique && (await Api.get('test')).data && (msg = 'HASH已存在');
  return msg;
});

// 验证端口
const port = (rule, value, callback) => validator(rule, value, callback, (value) => {
  let msg = '';
...
// 验证压缩包文件格式是否正确
const file = (rule, value, callback) => validator(rule, value, callback, (value) => {
  let file;
  let msg = '';
...
// 验证邮箱账号
const email = (rule, value, callback) => validator(rule, value, callback, async (value) => {
  if (!Tool.isEmail(value)) return '邮箱格式有误';
  if (rule.isUnique && (await Api.get('test')).data) return '邮箱已存在';
});
const emailArr = (rule, value, callback) => validator(rule, value, callback, (value) => {
 ...
 */
const taskTarget = (rule, value, callback) => validator(rule, value, callback, (value) => {
